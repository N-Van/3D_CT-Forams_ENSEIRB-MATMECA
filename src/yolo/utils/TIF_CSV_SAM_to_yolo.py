...
def save_images_and_annotations(tif_path, csv_path, output_folder, box_width, num_frames):
    ...
    for z_idx in tqdm(range(tif_data.shape[0]), desc="Processing images"):
        ...
        output_image_path = os.path.join(images_folder, f'image_z_{z_idx}.png')
        cv2.imwrite(output_image_path, img_array)

        # Write annotations to a file with the same name as the image in the labels folder
        annotation_file_path = os.path.join(labels_folder, f'image_z_{z_idx}.txt')
...
if __name__ == "__main__":
    ...
    save_images_and_annotations(args.tif_path, args.csv_path, args.output_folder, args.box_width, args.num_frames)
